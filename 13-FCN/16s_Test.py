# -*- coding: utf-8 -*-
"""
Created on Fri May  3 10:44:20 2019

@author: mates
"""
#IMPORTAR Y ASIGNAR LA CONFIGURACIÓN RESPECTIVA
import os
...
configurations = {
    ...
    1: dict(
        max_iteration=100000, #Max iteration
...
class FCN16s(nn.Module):
    ...
    def __init__(self, n_class=21): #Número de clases : 21
        super(FCN16s, self).__init__()
        # conv1
        self.conv1_1 = nn.Conv2d(3, 64, 3, padding=100) #Zero-padding to both sizes of input
...
#CARGAR EL MODELO Y EVALUARLO, LO PASAMOS A CUDA
model = FCN16s(n_class=21)
if cuda: model.to('cuda')
model.eval()

#Cargar un modelo CHECKPOINT 
resume = 'logs/FCN16s/checkpoint.pth.tar'
print('Loading checkpoint from: '+resume)
checkpoint = torch.load(resume)
model.load_state_dict(checkpoint['model_state_dict'])

#CORRER EL MODELO, FUNCIONES PARA CARGAR LAS IMÁGENES
import PIL.Image
import torch

def fileimg2model(img_file): #Abrir la imagen en PIL para meterla al modelo
    img = PIL.Image.open(img_file)
    img = np.array(img, dtype=np.uint8)
    return transform(img)

def transform(img): #La pasa a torch y normaliza los valores de la imagen, (rgb--> bgr y trasponer)
    img = img[:, :, ::-1]  # RGB -> BGR
...
def run_fromfile(img_file):
    #Pasar la imagen a torch
    img_torch = torch.unsqueeze(fileimg2model(img_file), 0)
    #Pasar la imagen a cuda (gpu)
    if cuda: img_torch = img_torch.to('cuda')
    with torch.no_grad():
        #El modelo me da un score
        score = model(img_torch)
        #Pasar los data a numpy,max del score por cada uno
